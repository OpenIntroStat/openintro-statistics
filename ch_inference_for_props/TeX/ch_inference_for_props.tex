...
\Add{The labels \emph{success} and \emph{failure} need not mean something positive or negative. The terms \term{success} and \term{failure} are just convenient words frequently used when discussing proportions.}
...
\item[Success-failure condition.] The sample size must also be sufficiently large, which is checked using the \hiddenterm{success-failure condition}. There were approximately $1042 \times \hat{p} = 854$ ``successes'' and $1042 \times (1 - \hat{p}) = 188$ ``failures'' in the sample, both easily greater than~10.
\end{description}
...
Estimate the standard error of $\hat{p}=0.44$ using Equation~\eqref{seOfPHat}. Because $p$ is unknown and the standard error is for a confidence interval, we use $\hat{p}$ in place of $p$.\footnote{$SE = \sqrt{\frac{p(1-p)}{n}} \approx \sqrt{\frac{0.82 (1 - 0.82)}{1042}} = 0.012$.}
\end{exercise}

\begin{example}{Construct a 95\% confidence interval for $p$, the proportion of New York adults who supported a quarantine for anyone who has come into contact with Ebola.}
Using the standard error estimate from Exercise~\ref{seOfPropOfAmericansJobApprovalOfSupremeCourt}, the point estimate 0.82, and $z^{\star} = 1.96$ for a 95\% confidence interval, the confidence interval may be computed as
\begin{eqnarray*}
\text{point estimate } \ \pm\ z^{\star}SE \quad\to\quad 0.82 \ \pm\ 1.96\times 0.012 \quad\to\quad (0.796, 0.844)
\end{eqnarray*}
We are 95\% confident that the true proportion of New York adults who supported a quarantine for anyone who has come into contact with an Ebola patient (in October 2014) is between 0.796 and 0.844.

\index{data!Ebola poll|)}

\end{example}

\Add{Notice that since the poll was around the time where a doctor in New York had come down with Ebola, the results may not be as applicable today as they were at the time the poll was taken. This highlights an important detail about polls: they provide data about public opinion at a single point in time.}
...
\begin{example}{A simple random sample of 1,028 US adults in March 2013 found that 56\% support nuclear arms reduction.\footnote{\oiRedirect{textbook-nuclear_arms_reduction_201303}{www.gallup.com/poll/161198/favor-russian-nuclear-arms-reductions.aspx}} Does this provide convincing evidence that a majority of Americans support nuclear arms reduction at the 5\% significance level?} \label{NuclearArmsInferenceExample}
The poll was of a simple random sample that includes fewer than 10\% of US adults, meaning the observations are independent. In a one-proportion hypothesis test, the success-failure condition is checked using the null proportion, $p_0 = 0.5$: $n p_0 = n (1 - p_0) = 1028 \times 0.5 = 514 > 10$. With these conditions verified, the normal model may be applied to $\hat{p}$.
...
\Add{In Example~\ref{qualityCtrlEngHypothesisEval}, the null difference was 0.03. However, in the vast majority of applications for differences in means or proportions, the null difference is~0. While the details for a difference of means does not change if the null difference is zero or non-zero, that is not the case for a difference in proportions. As we'll see next, a hypothesis test for a difference in proportions where the null value is 0 requires additional~care.}
...
\Comment{This subsection has been overhauled and contains a controversial topic, so it requires a careful review.}

\index{data!mammography|(}
\index{data!breast cancer|(}

A mammogram is an X-ray of a breast, and it is frequently used to check for breast cancer. Whether mammograms should be used is part of a controversial discussion, and it's the topic of our next example where we examine 2-proportion hypothesis test when $H_0$~is~$p_1 = p_2$.
...
In Example~\ref{condFormammogramStudySummaryTableNormalInference}, we check the conditions for using the normal model to analyze the results of the study. The details are very similar to what was completed in the previous sections. However, this time we use a special proportion called the \term{pooled proportion} to check the success-failure condition. This proportion, which in this example is the estimate of the breast cancer death rate across the entire study, is our best estimate of the proportions $p_{mgm}$ and $p_{ctrl}$ \emph{if the null hypothesis is true that $p_{mgm} = p_{ctrl}$}. We will also use the pooled proportion again when computing the standard error.
...
We also much check the success-failure condition for each group. Under the null hypothesis, the proportions $p_{mgm}$ and $p_{ctrl}$ are equal, so we check the success-failure condition with our best estimate of these values under $H_0$, the \term{pooled proportion} from the two samples:
\begin{align*}
\hat{p} = \frac{500 + 505}{500 + \text{44,425} + 505 + \text{44,405}} = 0.0112
\end{align*}
We then use $\hat{p}$ to check the success-failure condition for each group:
\begin{align*}
\hat{p} \times n_{mgm} &= 0.0112 \times \text{44,925} = 503
	& (1 - \hat{p}) \times n_{mgm} &= 0.9888 \times \text{44,925} = 44,422 \\
\hat{p} \times n_{ctrl} &= 0.0112 \times \text{44,910} = 503
	& (1 - \hat{p}) \times n_{ctrl} &= 0.9888 \times \text{44,910} = 44,407
\end{align*}
The success-failure condition is easily satisfied since all values are at least 10, and we can safely apply the normal model.
\end{example}

\begin{termBox}{\tBoxTitle{Use the pooled proportion estimate when $\mathbf{H_0: p_1 = p_2}$}
...
\begin{example}{Compute the point estimate of the difference in breast cancer death rates in the two groups, and use the pooled proportion $\hat{p} = 0.0112$ from Example~\ref{condFormammogramStudySummaryTableNormalInference} to calculate the standard error.}
...
\hat{p}_{mgm} - \hat{p}_{ctrl}
	...
	&= -0.00012
\end{align*}
There was a difference of about 0.012\% in death from breast cancer in the two groups. Next, the standard error is calculated using the pooled proportion, $\hat{p}$:
\begin{align*}
...
As like in past tests, we first compute a test statistic and draw a picture:
\begin{align*}
...
\begin{itemize}
\setlength{\itemsep}{0mm}
\item If mammograms are helpful or harmful, the data indicate that the effect isn't very large. So while we do not accept the null hypothesis, it is reasonable to say that mammograms do not reduce or increase breast cancer deaths by much. We could compute a confidence interval to give a sense of the plausible impact of mammograms.
\item Are mammograms more or less expensive than a non-mammogram breast exam? If one is much more expensive than the other without offering clear benefits, then we should lean towards the less expensive option.
\item The study also found that mammograms led to overdiagnosis of breast cancer, which means some breast cancers were found (or thought to be found) but that they would not cause symptoms during patients' lifetimes. That is, something else would kill the patient before breast cancer symptoms appeared. This means some patients may have been treated for breast cancer unnecessarily, and this treatment is another cost to consider. Additionally, overdiagnosis generally causes unnecessary physical or emotional harm to a patient.
\end{itemize}
