...
In this section, we'll explore the success-failure condition
and also encounter more statistics beyond $\hat{p}$ where
versions of the Central Limit Theorem apply.


\subsection{Why do we require $np \geq 10$ and
    $n(1-p) \geq 10$?}
...
To get a feel for whether a normal distribution is reasonable,
it can be helpful to make your eyes go a little out of focus
and see if the slightly blurred image of the plot looks normal.
While that is not a scientific approach, it does give us
some sense of when the sampling distribution of $\hat{p}$
can be reasonably approximated a normal distribution.
We recommend you take a moment to review
Figures~\ref{}
and~\ref{}.
In each case, we've reported both $np$ and $n(1 - p)$,
so you can evaluate for yourself what threshold,
if not the standard of \emph{10}, you think makes
the most sense.

Completing this exercise may leave you wondering,
\emph{is~10~enough?} To answer this question
...
start by considering different sample sizes.
