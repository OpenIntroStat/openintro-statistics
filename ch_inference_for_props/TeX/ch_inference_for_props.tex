...
\Add{In Example~\ref{qualityCtrlEngHypothesisEval}, the null difference was 0.03. However, in the vast majority of applications for differences in means or proportions, the null difference is~0. While the details for a difference of means does not change if the null difference is zero or non-zero, that is not the case for a difference in proportions. As we'll see next, a hypothesis test for a difference in proportions where the null value is 0 requires an additional~care.}
