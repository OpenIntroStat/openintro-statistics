...
\begin{nexample}{We consider an experiment for patients
  ...
  First, we check if each proportion separately can be
  ...
  \item
      ...
      independent.
  \end{itemize}
  With these conditions satisfied, the individual proportions
  can be modeled using the normal distribution.
  
  Second, the randomization of patients into each group
  also ensures the independence of observations across the
  two groups.
  
  Both of the required conditions are satisfied,
  so the difference in sample proportions can be
...
Next, we will conduct a hypothesis test when we model
$\hat{p}_1 - \hat{p}_2$ using a normal distribution.
%We will check the success-failure condition for each
%group and compute the standard error slightly differently,
%using what is called the
%\emph{pooled proportion}\index{pooled proportion}
%instead of the point estimates of each proportion separately,
%but otherwise the procedure will feel familiar.
We'll explore this method in the context of an experiment
evaluating the benefits of mammograms.
...
However, this time we use a special proportion called the
...
\begin{nexample}{Can we use a normal model to analyze this study?}
  ...
  \end{align*}
  The success-failure condition is satisfied since all values
...
\begin{nexample}{The quality control engineer from
  ...
  Since this is a one-sided test, this single tail area is also
  the p-value, and we reject the null hypothesis because 0.006
  is less than 0.05.
  That is, we have statistically significant evidence that the
  higher-quality blades actually do pass inspection more
  than 3\% as often as the currently used blades.
  Based on these results, management will approve the switch
  to the new supplier.
\end{nexample}
\end{examplewrap}
...
in two proportions is similar to the formula for other
standard errors.
