...
\Comment{The \data{email50} data set has been swapped out here for the \data{loan50} data set.}
...
\begin{tabular}{ccc ccc ccc}
  \hline
   & loan\_amount & interest\_rate & term & grade & state
       & total\_income & verified\_income & home\_ownership \\ 
  \hline
  1 & 7500 & 7.34 & 36 & A & MD & 70000 & Source Verified & RENT \\ 
  2 & 25000 & 9.43 & 60 & B & OH & 254000 & Not Verified & MORTGAGE \\ 
  3 & 14500 & 6.08 & 36 & A & MO & 80000 & Not Verified & MORTGAGE \\ 
  $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$
      & $\vdots$ & $\vdots$ & $\vdots$ \\
  50 & 3000 & 7.96 & 36 & A & CA & 34000 & Not Verified & RENT \\ 
   \hline
\end{tabular}
}
...
\label{loan50DF}
\end{figure}
% library(openintro); vars <- c("loan_amount", "interest_rate", "term", "grade", "state", "total_income", "verified_income", "home_ownership"); library(xtable); data(loan50); loan50[c(1,2,3,50), vars]; xtable(loan50[c(1,2,3,50), vars])
...
\Comment{We could drop the \data{county} data set from
    this section and use other examples.}

\index{data!county|(}

\begin{exercise}\label{desc_county_as_data_matrix}
We consider data for 3,143 counties in the United States,
which includes each county's name,
the state where it resides, its population in 2000 and 2010,
per capita federal spending, poverty rate,
and five additional characteristics.
...
a data matrix?\footnote{Each county may be viewed as a case,
  ...
  each case.
  A table with 3,143 rows and 11 columns could hold these data,
...
\Comment{Move to using the \data{loan50} data set here, then use the \data{county} data set as example / practice.}

% For 'county' Guided Practice:
% \var{fed\_\hspace{0.3mm}spend}
% \var{pop2010}
% \var{state}
% \var{smoking\_\hspace{0.3mm}ban}
...
Most often, discrete numerical variables are those where
...
Are these numerical or categorical variables?\footnote{There
...
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item[(1)]\label{interest_rate_vs_loan_size}
