...
\Comment{This section and its data were overhauled.
  The most conspicuous change was removing the
  \data{email50} data set for the \data{loan50} data set.
  US county-level data has also been updated for variables
  where new data was available.}

Effective presentation and description of data is a first step
in most analyses.
This section introduces the \emph{data matrix} for organizing
data as well as some terminology that will be used throughout
this book.
...
\Comment{Fix this reference and move it to the data appendix
    when it is
    ready.}%
\footnote{\oiRedirect{textbook-census_quick_facts}
    {quickfacts.census.gov/qfd/index.html}}

\Comment{Fix weird page break behavior here.}
...
\var{multi\us{}unit}  &
    ...
    e.g. apartments \\
\var{unemp\us{}rate} & Unemployment rate \\
...
\Comment{Watch the line break here with the GP footnote.
    In fact, watch for this carefully throughout the entire book
    when doing final formatting.}
...
and \var{num\us{}migraines} represents the number of migraines the
...
\begin{figure}
  \centering
  \Figure{0.8}{pop_change_v_med_income}
...
\begin{figure}[ht]
  \centering
  \Figures{0.45}{popToSample}{popToSampleGraduates}
...
\begin{figure}
  \centering
  \Figures{0.45}{popToSample}{popToSubSampleGraduates}
...
\begin{center}
  \Figures{0.6}{variables}{sunCausesCancer}
\end{center}
% Some studies:
...
a group of similar individuals over many years to assess
the possible influences of behavior on cancer risk.
One example of such a study is The Nurses' Health Study,
started in 1976 and expanded in 1989.\footnote{\oiRedirect{textbook-channing_nurse_study}{www.channing.harvard.edu/nhs}}
...
Some data sets, such as \data{county}, may contain both
prospectively- and retrospectively-collected variables.
Local governments prospectively collect some variables
as events unfolded (e.g. retails sales) while the federal
government retrospectively collected others such as during
a census.
