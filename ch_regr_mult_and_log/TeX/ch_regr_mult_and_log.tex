...
\begin{example}{Results corresponding to the \emph{full model} for the \data{mario\_\hspace{0.3mm}kart} data are shown in Figure~\ref{outputForMultipleRegrOutputForAllPredictors2}. How should we proceed under the backward elimination strategy?} \label{backwardEliminationExampleWMarioKartData}
...
\begin{tipBox}{\tBoxTitle{When to use the adjusted $R^2$}
...
\begin{tipBox}{\tBoxTitle{When to use the p-value approach}
...
\emph{multiple comparison}\index{multiple comparison}
correction.
...
\Comment{The two tip boxes above previously were one box,
    and the discussion of multiple comparison correction
    in the second box was added.}
