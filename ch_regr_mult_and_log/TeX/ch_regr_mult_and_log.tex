...
\Comment{We will be reaching out to groups that work on
topics of racial and gender discrimination to provide
feedback on the presentation of the material in this
section.
...
Race and gender are \term{protected classes} in the
...
\begin{nexample}{We start by fitting a model with a single
    predictor: \var{employment\us{}holes}.
...
To convert from values on the regression-scale
...
e.g. \hiddenterm{Akaike information criterion (AIC)}%
\index{AIC|textbf}.
...
we ultimately eliminate the \var{college\us{}degree},
\var{military}, \var{email\us{}address}, and \var{gender}
predictors.
...
\begin{tabular}{rrrrr}
  ...
  \hline
  \vspace{-3.8mm} & & & & \\
  (Intercept) & -2.7727 & 0.1344 & -20.63 & 0.0000 \\ 
  job\_cityChicago & -0.3504 & 0.1089 & -3.22 & 0.0013 \\ 
  years\_experience & 0.0264 & 0.0096 & 2.76 & 0.0059 \\ 
  honors & 0.7935 & 0.1829 & 4.34 & 0.0000 \\ 
  racewhite & 0.4399 & 0.1079 & 4.08 & 0.0000 \\ 
\hline
\end{tabular}
...
% % m <- glm(callback ~ job_city + years_experience + honors + race, data = d, family = binomial); summary(m); xtable(m)
\newcommand{\resRaceWhiteCoefReduced}{0.4399}

\begin{exercisewrap}
\begin{nexercise}
The \var{race} variable had taken only two levels:
\resp{black} and \resp{white}.
Based on the model results, was race a meaningful
factor for if a prospective employer would
call back?\footnotemark
\end{nexercise}
\end{exercisewrap}
\footnotetext{The coefficient shown in the model for the
    race variable reflects the log-odds gain for resumes
    where the candidate's first name implied they were
    white.
...
\begin{nexample}{Examine the summary of the reduced model
    ...
    and in particular, examine the \var{racewhite} row.
...
We previously saw this in the Ebay auction example when we compared the coefficient of \var{cond\_\hspace{0.3mm}new} in a single-variable model and the corresponding coefficient in the multiple regression model that used three additional variables (see Sections~\ref{twoSingleVariableModelsForMarioKartData} and~\ref{includingAndAssessingManyVariablesInAModel}).
...
\begin{nexample}{Use the model to estimate the probability
  ...
  \begin{itemize}
  \setlength{\itemsep}{0mm}
  %\item \var{(Intercept)}: Always included.
  \item \var{job\us{}cityChicago}:
      Since the resume is for a position in Chicago,
      we will include this coefficient.
  \item \var{years\us{}experience}:
