...
$\vdots$ &$\vdots$ &$\vdots$ &$\vdots$ &$\vdots$ &$\vdots$ \\
  ...
  141 & 54.51 &  1 &  1 & 1 &   2 \\ 
   \hline
\end{tabular}
\caption{Four observations from the \data{mario\us{}kart} data set.}
...
(Intercept) & 42.8711 & 0.8140 & 52.67 & 0.0000 \\ 
  cond\us{}new & 10.8996 & 1.2583 & 8.66 & 0.0000 \\ 
   \hline
...
Note that \var{cond\us{}new} is a two-level categorical variable that takes value 1 when the game is new and value 0 when the game is used. So 10.90 means that the model predicts an extra \$10.90 for those games that are new versus those that are used. (See Section~\ref{categoricalPredictorsWithTwoLevels} for a review of the interpretation for two-level categorical predictor variables.) Examining the regression output in Figure~\ref{singleVarModelsForPriceUsingCond}, we can see that the p-value for \var{cond\us{}new} is very close to zero, indicating there is strong evidence that the coefficient is different from zero when using this simple one-variable model.
\end{nexample}
\end{examplewrap}
...
We want to construct a model that accounts for not only the game condition, as in Section~\ref{twoSingleVariableModelsForMarioKartData}, but simultaneously accounts for three other variables: \var{stock\us{}photo}, \var{duration}, and \var{wheels}.
\begin{align}
\widehat{\var{price}}
...
(Intercept) & 36.2110 & 1.5140 & 23.92 & 0.0000 \\ 
  cond\us{}new & 5.1306 & 1.0511 & 4.88 & 0.0000 \\ 
...
(Intercept) & 36.0483 & 0.9745 & 36.99 & 0.0000 \\ 
  cond\us{}new & 5.1763 & 0.9961 & 5.20 & 0.0000 \\ 
...
callback rates.
Researchers monitored job postings in Boston and Chicago for
several months and used this to build up a set of test cases.
To evaluate which factors were important, these researchers
created many fake resumes to send off to these jobs.
They enumerated important characteristics, such as years of
experience and education details, and they used these
characteristics to randomly generate a resume.
Finally, they randomly assigned a name to the resume, where
the name would imply the applicant's gender and race.
...
with special interest in the race and gender variables.
...
Importantly, we can infer causation between these
...
it will start to feel like we're back in the multiple
regression context,
...
predictors, giving the smaller model summarized in
...
\begin{tabular}{rrrrr}
  ...
  job\us{}cityChicago & -0.3504 & 0.1089 & -3.22 & 0.0013 \\ 
  years\us{}experience & 0.0264 & 0.0096 & 2.76 & 0.0059 \\ 
...
\begin{nexample}{Compare the coefficient of the
  ...
  unchanged even after we correct for these other variables.
...
\begin{nexample}{Compute the probability of a callback
    ...
    to be from a black individual but who otherwise
  ...
  a white-sounding name would need to
  apply to $\frac{1}{0.0653} = 15.3$ jobs on average
  to receive a callback,
  while an individual with an identical resume but with
  a black-sounding name would need
  to apply to $\frac{1}{0.0430} = 23.3$ jobs on average
  to receive a callback.
  ...
  than a person with a white-sounding name.
\end{nexample}
\end{examplewrap}

\Comment{Will the phrases \emph{white-sounding}
    or \emph{black-sounding} in the above example
    be perceived as offensive by some individuals?
    This aspect needs to be thoughtfully reviewed,
    and if the answer is \emph{yes}, revised.}
...
Reverse discrimination against a majority
group is likely real in some situations.
Yet, the impact of this reverse discrimination
appears to be so much smaller than the discrimination
experienced by minorities... \emph{why?}
...
\begin{nexample}{Let's consider a gender-imbalanced
  ...
  Of the 500 colleagues, men will experience
...
against the other demographic, it is the minority groups
that will greater experience those effects.
