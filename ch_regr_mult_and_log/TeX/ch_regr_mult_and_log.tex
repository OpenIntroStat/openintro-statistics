...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{ch_regr_mult_and_log/figures/marioKartSingle/marioKartSingle}
...
\Add{Adjusted $R^2$ could have been used in Chapter~\ref{linRegrForTwoVar}. However, when there is only $k = 1$ predictors, adjusted $R^2$ is very close to regular $R^2$, so this nuance isn't typically important when considering only one predictor.}
...
\Comment{Section 8.3 will also need significant revisions.}
...
\Cut{In this section, and in} \Add{In} practice, the model that includes all available explanatory variables is often referred to as the \term{full model}. Our goal is to assess whether the full model is the best model. If it isn't, we want to identify a smaller model that is preferable.
...
We compare the adjusted $R^2$ of each model to determine which to choose. Since the first model has an $R^2_{adj} = $ smaller than the $R^2_{adj}$ of the second model ($R^2_{adj} = $), we would prefer the second model to the first.
\end{example}
...
Two common strategies for adding or removing variables in a multiple regression model are called \emph{backward-selection} and \emph{forward-selection}. These techniques are often referred to as \term{stepwise} model selection strategies, because they add or delete one variable at a time as they ``step'' through the candidate predictors.
...
Our baseline adjusted $R^2$ is from the full model: $R^2_{adj} = 0.7108$, and we need to determine whether dropping a predictor will improve the adjusted $R^2$. To check, we fit four models that each drop a different predictor, and we retrieve the adjusted $R^2$ from each:
\begin{center}
\begin{tabular}{lllll}
Exclude ... &
...
Since we eliminated a predictor from the model in the first step, we see whether we should eliminate any additional predictors. Our baseline adjusted $R^2$ is now $R^2_{adj} = 0.7128$. We now fit three new models, which each consider eliminating each of the three remaining predictors:
\begin{center}
\begin{tabular}{llll}
...
\end{tabular}
\end{center}
None of these models lead to an improvement in adjusted $R^2$, so we do not eliminate any of the predictors. That is, after backward-selection, we are left with the model that keeps \var{cond\_\hspace{0.3mm}new}, \var{stock\_\hspace{0.3mm}photos}, and \var{wheels}, which we can summarize using the coefficients from Table~\ref{marioKartMultipleRegressionModelAllButDuration} on page~\pageref{marioKartMultipleRegressionModelAllButDuration}:
\begin{align*}
\hat{y} \ &= \ b_0 + b_1x_1 + b_2x_2 + b_4x_4 \\
	&= \ 36.05 + 5.18x_1 + 1.12x_2 + 7.30x_4
\end{align*}
where $x_1$ represents \var{cond\_\hspace{0.3mm}new}, $x_2$ represents \var{stock\_\hspace{0.3mm}photo}, and $x_4$ represents \var{wheels}.
\end{example}
...
\end{tabular}
\end{center}
In this case, we compare the adjusted $R^2$ against a baseline model that has no predictors: $R^2_{adj} = 0.0000$. The model with one predictor that has the largest adjusted $R^2$ is the model with the \var{wheels} predictor, and because this adjusted $R^2$ is larger than the baseline of 0.0000, we will add this variable to our model.
...
Because we have again added a predictor, we check whether adding the last variable, \var{duration}, will improve adjusted $R^2$. We compare the adjusted $R^2$ for model that includes \var{duration} along with the other three predictors (0.7108) to the model that only considers \var{wheels}, \var{cond\_\hspace{0.3mm}new}, and \var{stock\_\hspace{0.3mm}photo} (0.7128). Adding \var{duration} does not improve the adjusted $R^2$, so we do not add it to the model, and we have arrived at the same model that we identified from backward-selection:

\end{example}

%\begin{example}{We could have used p-value instead of the adjusted $R^2$ criteria in selecting variables for the model. Rather than look for the model with the largest adjusted $R^2$, we could have used forward or backward selection with p-values:
%added variables with the smallest p-value (forward-selection) or dropped variables with the largest p-values (backward-selection) until we were left with , we look for the model with the largest $R_{adj}^2$. What would the result of forward-selection be using the adjusted $R^2$ approach?}
...
There is no guarantee that the backward-elimination and forward-selection strategies will arrive at the same final model. If the backwards-elimination and forward-selection strategies are both tried and they arrive at different models, choose the model with the larger $R_{adj}^2$; other tie-break options exist but are beyond the scope of this book.
}
...
When the sole goal is to improve prediction accuracy, use adjusted $R^2$. This is commonly the case in a machine learning applications.\\[-2mm]

When there is interest in producing a simpler model, even if the final model's predictions are slightly less accurate, then the p-value approach is preferred. For example, this is common in business contexts where the model will be presented to non-statisticians.}
\end{tipBox}
...
\Comment{Will we want some end-of-chapter exercises on when to use $R^2_{adj}$ or the p-value?}
}
...
\begin{figure}
\centering
\includegraphics[width=0.72\textwidth]{ch_regr_mult_and_log/figures/marioKartDiagnostics/mkDiagnosticEvsAbsF}
...
\begin{figure}
\centering
\includegraphics[width=0.85\textwidth]{ch_regr_mult_and_log/figures/marioKartDiagnostics/mkDiagnosticEvsVariables}
