\documentclass[10pt,openany]%,oneside]
{book}

\usepackage{csquotes}

\usepackage{
  amsmath, calc,
  caption, changepage,
  endnotes, enumerate,
  graphicx,
  epstopdf,
  fancyhdr,
  fix-cm,
  fncychap,
  footmisc,
  fullminipage,
  geometry,
  ifthen, lscape,
  makeidx, manfnt,
  marginnote,
  mdframed,
  multicol, multirow,
  setspace, soul,
  tabto,
  textcomp,
  varioref, verbatim,
  wasysym, wrapfig
}
\usepackage[subfigure]{tocloft}

\usepackage{subfigure}
\usepackage[explicit]{titlesec}


% Croatian
\newcommand{\versiondate}[0]{13. studeni 2019}

\usepackage[T1]{fontenc}
\usepackage[croatian, safe=none]{babel}

\addto{\captionscroatian}{\renewcommand*{\appendixname}{Prilog}}
\addto{\captionscroatian}{\renewcommand*{\chaptername}{Glava}}

% English
%\newcommand{\versiondate}[0]{13 November 2019}
%
%\usepackage[T1]{fontenc}


%\usepackage[usenames,dvipsnames]{color}
%\newcommand{\href}[2]{#2} \newcommand{\url}[1]{#1} \newcommand{\urlstyle}[1]{}
\input{extraTeX/style/colorsV1}
\newcommand{\printlocation}[0]{}

% _____ PDF _____ %
\usepackage[bookmarksnumbered, colorlinks = false, pdfborder = {{0 0 0}}, urlcolor = oiGB, colorlinks=true, linkcolor = oiGB, citecolor = oiGB, backref = false]{hyperref}

% _____ Paperback _____ %
%\definecolor{oiB}{rgb}{0,0,0}\definecolor{chaptertitlegray}{rgb}{0,0,0}\usepackage[colorlinks=false,pdfborder={0 0 0},urlcolor= black,colorlinks=black,linkcolor=black, citecolor=black,backref=true]{hyperref}


% _____ Hardcover _____ %
%\definecolor{oiB}{rgb}{0,0,0}\definecolor{chaptertitlegray}{rgb}{0,0,0}\usepackage[colorlinks=false,pdfborder={0 0 0},urlcolor= black,colorlinks=black,linkcolor=black, citecolor=black,backref=true]{hyperref}
% !!!!!
%
% Also must \include{extraTeX/style/hardcover} below.
%
% !!!!!


% _____ Color Paperback _____ %
%\usepackage[colorlinks=false,pdfborder={0 0 0},urlcolor= black,colorlinks=black,linkcolor=black, citecolor=black,backref=true]{hyperref}
%\renewcommand{\printlocation}[0]{\noindent Printed in China. \\}

\usepackage[style=authortitle-ibid, maxnames=2,natbib=true,sortcites=true,block=space,backend=bibtex]{biblatex}
\bibliography{eoce.bib}

\makeindex
\newlength{\captionwidth}   % dodano jer style.tex kaže da ne postoji ta komanda
\input{extraTeX/style/style}
%\input{extraTeX/style/tablet}
%\include{extraTeX/style/video}
% The following style file supports an 8.25 x 11 paper size.
%\include{extraTeX/style/hardcover}

% Croatian
\input{extraTeX/preamble/title_derivative}
% English
%\input{extraTeX/preamble/title}

\date{}
%Croatian
\renewcommand\contentsname{Sadržaj}
\setcounter{tocdepth}{1}

\renewcommand{\cftchapfont}{\scshape}
\renewcommand{\cftsecfont}{\bfseries}

\begin{document}
%\include{extraTeX/preamble/review_copy}
\renewcommand{\thepage}{}
\maketitle

%Croatian
\include{extraTeX/preamble/copyright_derivative}

%English
%\include{extraTeX/preamble/copyright}

\renewcommand{\thepage}{\arabic{page}}
\tableofcontents
\include{extraTeX/preamble/preface}
\normalsize

\begingroup
\input{extraTeX/style/headers}
\includechapter{1}{ch_intro_to_data}
%\includechapter{2}{ch_summarizing_data}
%\includechapter{3}{ch_probability}
%\includechapter{4}{ch_distributions}
%\includechapter{5}{ch_foundations_for_inf}
%\includechapter{6}{ch_inference_for_props}
%\includechapter{7}{ch_inference_for_means}
%\includechapter{8}{ch_regr_simple_linear}
%\includechapter{9}{ch_regr_mult_and_log}
\endgroup

\begingroup
\include{extraTeX/style/style_appendices}
\appendix{}
\addtocontents{toc}{\protect\setcounter{tocdepth}{0}}\include{extraTeX/eoceSolutions/eoceSolutions}
\include{extraTeX/data/data}
\include{extraTeX/tables/TeX/zTable}
\include{extraTeX/tables/TeX/tTable}
\include{extraTeX/tables/TeX/chiSquareTable}
\endgroup

\include{extraTeX/index/index}
\printindex

\end{document}