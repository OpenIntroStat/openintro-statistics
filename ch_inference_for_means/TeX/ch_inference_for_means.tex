...
We examined 100 courses, and of those,
34 required books that could be found on Amazon.
...
\begin{tabular}{r ll ccc}
 ...
 & subject &
  ...
  33 & Theater & 35A & 19.99 & 17.00 & 2.99 \\ 
  34 & World Arts and Cultures & M23 & 47.97 & 47.45 & 0.52 \\ 
  \hline
\end{tabular}
\caption{Five cases of the
    \data{ucla\_\hspace{0.3mm}textbooks} data set.}
\label{textbooksDF}
\end{figure}
...
observations~2 and~3.\footnote{Observation~2:
  $14.26 - 13.55 = 0.71$.
...
\caption{Summary statistics for the price differences.
    There were 34 books, so there are 34 differences.}
...
In fact, the data are so strongly skewed that
we should make a note in our final result,
even though the sample is larger than 30 ($n = 34$).
\Comment{This is tricky to get the wording right.
  We'd probably want to be a bit doubtful of any
  statistically significant results given the degree
  of the skew.
  The best solution is probably to double the
  sample size...}
...
T
  ...
  = \frac{2.98 - 0}{2.85} = 1.05
\end{align*}
The degrees of freedom are $df = 34 - 1 = 33$.
If we examined Appendix~\vref{tDistributionTable},
we would see that this value is smaller than the left row,
which indicates that the two-tail area and p-value is
greater than 0.20.
If we used statistical software, we would find the p-value
is about 0.301.
...
%We (the authors) found this result interesting.
%Amazon prices were almost uniformly lower than those
%of the UCLA bookstore's in a 2010 sample we collected.
%Today, the UCLA bookstore is about even with Amazon
%on the vast majority of titles, and there is no
%statistical difference in the sample.
%If we examine the data, we do see that there are
%occasions where Amazon is much better than the
%UCLA Bookstore.
%However, those cases are uncommon.
%Additionally, today there are many instances where
%the UCLA Bookstore has lower prices than those
%offered by Amazon, which was rare in our previous
%data set from 2010.
