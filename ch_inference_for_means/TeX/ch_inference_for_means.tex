...
The motivation in Chapter~\ref{foundationsForInference} for requiring a large sample was two-fold:
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\end{enumerate}
The $t$-distribution is a helpful substitute for the normal distribution when we model a sample mean $\bar{x}$ that comes from a smaller sample. While we emphasize the use of the $t$-distribution for small samples, this distribution is also generally used for large samples and will produce similar results to those from the normal distribution for large samples.
...
It's very useful to become familiar with the $t$-distribution, because it allows us greater flexibility than the normal distribution when analyzing numerical data. We use a \term{t table}, partially shown in Table~\ref{tTableSample}, in place of the normal probability table. A larger $t$-table is in Appendix~\ref{tDistributionTable} on page~\pageref{tDistributionTable}. In~practice, it's standard practice to use statistical software instead of a table, and you can see some of these options at
\begin{center}
\href{https://openintro.org/stat/prob-tables}{www.openintro.org/stat/prob-tables}
\end{center}
\Comment{Still need to create redirect and add this note elsewhere in the textbook.}
...
\Comment{{\small Subsection title was previously ``The $t$-distribution as a solution to the standard error problem''}}
...
Is the typical US runner getting faster or slower over time? We consider this question in the context of the Cherry Blossom Race, which is a 10-mile race in Washington, DC each~spring.\footnote{\urlwofont{http://www.cherryblossom.org}} %We'll compare runners in 2006 and 2012.
...
%\section{Consult: planning an experiment}
\label{PowerForDifferenceOfTwoMeans}

%So far we've been focused on controlling the Type~1 Error rate for hypothesis tests. However, when planning an experiment, we often are interested in detecting any real effect, so we should also be concerned about the size of the Type~2 Error rate.
Often times in experiment planning, there are often two competing considerations:
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\end{itemize}
%The first point is relatively simple: the more data we collect, the more precise our estimates and the smaller effects we can typically detect. The second point is more subtle, since we need to determine the size of effects that we care about.
...
%\begin{example}{Suppose a company has developed a new drug for lowering blood pressure and are preparing a clinical trial to determine its efficacy. They are targeting patients with systolic blood pressure between 140 and 180 mmHg. They have brought you in as a statistician for the project. What are some of the questions you should ask?}
%As a statistician, you may not generally be an expert in the application, so asking questions -- and a lot of questions -- is very important to better understand the context and the data to expect. Below are small sample of questions that could be asked during a first meeting about the project:
%\begin{description}
%\setlength{\itemsep}{0mm}
%\item[What is the main aim of the study? Who will I be working with] It's important to ask basic question to get a firm understanding of the context.
%\item[What side effects do you suspect may arise that we should monitor?] We may be well aware of the main outcome of interest, systolic blood pressure. However, it's also important to think about other data that should be collected.
%\item[Who is eligible for the study?] Asking about the study subjects is important. We'd also want to ask some numerical questions about who will be recruited, e.g. get a sense for the approximate average and standard deviation of patients on key variables such as systolic blood pressure.
%\item[What is the study timeline?] Understanding a deadlines is important. It's common for a statistician to be brought in at the last minute. Additionally, it's common for the amount of work required is underestimated, so it's important to push back on unrealistic timelines or get back to the researcher if the statistician determines the proposed timeline is unrealistic.
%\item[Who will need to review the study before it goes forward?] Clinical trials generally have review boards that are generally in place to ensure ethical research. Knowing who will be reviewing the plan can offer guidance on what the responsibilities will be for the statistician.
%\end{description}
%\end{example}
...
We're going to go through the motions of a hypothesis test. While this may not seem helpful at first, it will help us frame our calculations for determining an appropriate sample size for the study.

\begin{example}{Suppose a company has developed a new drug for lowering blood pressure, and they are preparing a clinical trial (experiment) to test the drug's effectiveness. They recruit people who are taking a particular standard blood pressure medication. People in the control group will continue to take their current medication through with generic-looking pills to ensure treatment blinding. Write down the hypotheses for a two-sided hypothesis test in this context.}
...
\item[$H_A$:] The new drug's performance differs from the standard medication. \\
  $\mu_{trmt} - \mu_{ctrl} \neq 0$.
\end{description}
Some researchers might argue for a one-sided test here, where the alternative would consider only whether the new drug performs better than the standard medication. However, it would be very informative to know whether the new drug performs worse than the standard medication, so we use a two-sided test to consider this possibility during an analysis.
\end{example}

\begin{example}{The company's researchers would like to run the clinical trial on patients with systolic blood pressures between 140 and 180~mmHg. Suppose the study's doctors expect the standard deviation of the patients' blood pressures to be about 12~mmHg and the distribution of patient blood pressures to be approximately symmetric.\footnote{In this particular study, we'd generally measure each patient's blood pressure at the beginning and end of the study, and then the outcome measurement for the study would be the average change in blood pressure. That is, both $\mu_{trmt}$ and $\mu_{ctrl}$ would represent average differences. This is what you might think of as a 2-sample paired testing structure, and we'd analyze it exactly just like a hypothesis test for a difference in the average change for patients. In the calculations we perform here, we'll suppose that 12~mmHg is the predicted standard deviation of a patient's blood pressure difference over the course of the study.} If~we had 100 patients per group, what would be the approximate standard error for $\bar{x}_{trmt} - \bar{x}_{ctrl}$?}
...
SE_{\bar{x}_{trmt} - \bar{x}_{ctrl}}
  ...
  = 1.70
\end{align*}
Since the doctors have only taken an educated guess at the standard deviation for the patients who will enroll, and the treatment may affect the variability of the patients, this may be an imperfect estimate of $SE_{\bar{x}_{trmt} - \bar{x}_{ctrl}}$. However, it is sufficient for our purposes.
\end{example}

\begin{example}{What does the null distribution of $\bar{x}_{trmt} - \bar{x}_{ctrl}$ look like?}
The degrees of freedom are greater than 30, so the distribution of $\bar{x}_{trmt} - \bar{x}_{ctrl}$ will be approximately normal. Its standard deviation (the standard error) would be about 1.70, and under the null hypothesis, its mean would be 0.
\begin{center}
\includegraphics[width=0.8\textwidth]{ch_inference_for_means/figures/power_null_0_1-7/power_null_A_0_1-7}
\end{center}
\end{example}
...
\begin{description}
\setlength{\itemsep}{0mm}
\item[Lower 2.5\%:] For the normal model, this is 1.96 standard errors below 0, so any difference smaller than $-1.96 \times 1.70 = -3.332$.
\item[Upper 2.5\%:] For the normal model, this is 1.96 standard errors above 0, so any difference larger than $1.96 \times 1.70 = 3.332$.
\end{description}
The boundaries of these \term{rejection regions} are shown below:
\begin{center}
\includegraphics[width=0.8\textwidth]{ch_inference_for_means/figures/power_null_0_1-7/power_null_B_0_1-7_with_rejection_region}
\end{center}
\end{example}
...
\begin{example}{Suppose we decided to move forward with 100 patients per treatment group and the new drug reduces blood pressure by an additional 3~mmHg relative to the standard medication. What is the probability that we detect a drop?}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{ch_inference_for_means/figures/power_null_0_1-7/power_null_C_0_1-7_with_alt_at_3}
\end{center}
\item The rejection regions, which is outside of the dotted lines above.
\item A little elbow grease to calculate the fraction of the sampling distribution that falls in the rejection region.
\end{itemize}
In short, we need to calculate the probability that $x < -3.332$ for a normal distribution with mean -3 and standard deviation 1.7. (We'll ignore the upper tail, which is in the opposite direction of the truth, if the true difference is indeed -3.) To do so, we first shade the area we want to calculate:
\begin{center}
\includegraphics[width=0.8\textwidth]{ch_inference_for_means/figures/power_null_0_1-7/power_null_D_0_1-7_with_alt_at_3_and_shaded}
\end{center}
...
\item The company probably invested hundreds of millions of dollars in developing the new drug, so now they are left with great uncertainty about its potential since the experiment didn't have a great shot at detecting effects that would still be important.
...
\item Another clinical trial is likely to be run to get a more conclusive answer as to whether the drug does hold any practical value, and conducting a second clinical trial may take years and many millions of dollars.
\end{itemize}
...
\includegraphics[width=0.7\textwidth]{ch_inference_for_means/figures/power_null_0_0-76/power_null_0_0-76_with_alt_at_3_and_shaded} \\
...
\begin{center}
\includegraphics[width=0.8\textwidth]{ch_inference_for_means/figures/power_best_sample_size/power_best_sample_size}
\end{center}
...
\begin{exercise}
What considerations do you think would be important in selecting power?\footnote{Answers will vary, but here are a few important considerations:
\begin{itemize}
\setlength{\itemsep}{0mm}
