...
\begin{figure}[h]
  \centering
  \Figures{0.45}{textbooksF18}{diffInTextbookPricesF18}
...
\begin{onebox}{Using the $t$-distribution for a difference in means}
  ...
  \item
      each sample mean separately follows a normal model.
  \end{itemize}
\end{onebox}

We can simplify the checking of the two conditions
by reorganizing them as a broader check for independence
and a check for outliers in each group:
\begin{description}
\item[Independence, extended.]
    The data are independent within and between
    the two groups, e.g. the data come from two
...
\begin{figure}[h]
  \centering
  \Figure{0.55}{stemCellTherapyForHearts}
