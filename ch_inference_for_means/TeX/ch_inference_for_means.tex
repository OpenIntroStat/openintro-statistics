...
\begin{nexample}{Suppose a pharmaceutical company has developed
  ...
  where the alternative would consider only whether the
  new drug performs better than the standard medication.
  However, it would be very informative to know whether
  the new drug performs worse than the standard medication,
  so we use a two-sided test to consider this possibility
  during the analysis.
\end{nexample}
\end{examplewrap}
...
\begin{nexample}{The researchers would like to run the clinical
    ...
    pressures will be approximately symmetric.\footnote{In
      this particular study, we'd generally measure each
      patient's blood pressure at the beginning and end
  ...
  be correct for this group of patients.
...
\begin{nexample}{Suppose we decided to move forward with
  ...
  \end{center}
  Then we calculate the T-score and find the tail area using
  either statistical software or the probability table with
  $df = 99$
  (from the minimum of $n_1 - 1$ and $n_2 - 1$):
  \begin{align*}
  T = \frac{-3.332 - (-3)}{1.7} = -0.20 \qquad \to \qquad 0.4207
  \end{align*}
...
\footnotetext{(a) The standard error is given as
  ...
  $T = \frac{-1.49 - (-3)}{0.76} = 1.99 \to 0.9767$
...
Instead, we should solve the problem backwards.

\begin{examplewrap}
\begin{nexample}{What sample size will lead to a power of 80\%?}
  We start by identifying the T-score that would give us a lower
  tail of 80\%.
  For a moderately large sample size per group,
  the T-score for a lower tail of 80\% would be about $T = 0.84$.
  ...
  \end{center}
  Additionally, the rejection region always extends
  $1.96\times SE$ from the center of the null distribution
  for $\alpha = 0.05$
  (again supposing a larger sample size to get the value of 1.96).
  ...
  \end{align*}
  In our example, we also want the distance between the null
  ...
  sample size.
  The revised sample size target will generally be a little
  larger.
\end{nexample}
\end{examplewrap}
...
\end{nexercise}
\end{exercisewrap}
\footnotetext{First, find the T-score such that 90\% of the
  distribution is below it: $Z = 1.28$ (assume large $df$).
