...
\item
    ...
    are a bit more complex, and we'll spend
    Section~\ref{x_bar_conditions} discussing
    rules of thumb.
\item
    The standard error is dependent on the population
    standard deviation, $\sigma$.
    However, we generally do not know $\sigma$ perfectly.
    A new distribution, called the $t$-distribution,
    will play a critical role in fixing this problem
    and is discussed in
...
\noindent%
Two conditions are nestled into the
...
\item[Normality.]
    ...
    This condition is vague and very difficult to evaluate
    for small samples (when it is most important), so we next
    provide a rule of thumb for checking this condition.
\end{description}
...
%  and another for large samples ($n \geq 30$):
  ...
  \item
      ...
      then the normality condition is satisfied.
  \item
...
\begin{nexample}{Consider the following two plots
    ...
    \end{center}
    Is it reasonable to model $\bar{x}$ using a normal distribution
    in each of these cases?}
  \label{outliers_and_ss_condition_ex}%
  Both samples are said to be from a simple random sample
  on their respective populations, so the independence condition
  is satisfied. Let's next check the normality condition for
  each using the rule of thumb.
  
  The first sample has fewer than 30 observations,
  so we are watching for any clear outliers.
  None are present, so the normality condition
  is reasonably met and it is reasonable to model
  $\bar{x}_1$ using a normal distribution.
...
followers for anyone who has ever posted on Twitter.
Most such individuals will have built up relatively
few followers, while others will have amassed
