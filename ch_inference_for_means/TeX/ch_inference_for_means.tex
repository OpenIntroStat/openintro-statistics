...
\noindent%
There are two conditions noted above for the
Central Limit Theorem\index{Central Limit Theorem}
to apply to $\bar{x}$.
\begin{description}
\item[Independence.]
    First, the sample observations must be independent,
    such as a simple random sample from a population.
    Second, we also must check that the sample size is
    sufficiently large for $\bar{x}$ to be nearly normal.
    This second condition is more nuanced and depends
    on the population distribution itself.
\item[Sample size.]
    The true requirement is that the sample comes from
    a normally distributed population,
    where we can relax this condition for larger sample sizes.
    This is obviously a vague guideline,
    so we next provide a rule of thumb as a substitute.
\end{description}
...
\begin{onebox}{Rule of thumb for $\bar{x}$ sample size check}
  If there are no clear outliers in the data,
  %and we don't
  %have reason to believe particularly extreme outliers are
  %present in population,
  then the sample size condition is satisfied.
  \stdvspace{}

  If the sample size is at least $n = 30$ observations
  and there are no \emph{particularly extreme} outliers,
  then the sample size condition is satisfied.
\end{onebox}

The sample size check is nuanced, even with the rule of thumb.
In this first course in statistics, you aren't expected
to develop perfect judgement on this condition.
However, you are expected to be able to handle the
clear cut cases based on the core guidelines,
which we introduce next to introduce the notion of
a particularly extreme outlier.

\begin{examplewrap}
\begin{nexample}{Consider these two plots,
    which have sample sizes of 15 and 50:
    \begin{center}
    \Figure{0.85}{outliers_and_ss_condition}
    \end{center}
    Evaluate whether the sample size condition satisfied
    in each case.}
  \label{outliers_and_ss_condition_ex}%
  The first sample is relatively small ($n = 15$),
  and no clear outliers are visible.
  In this case, the sample size condition is reasonably met.

  The second sample includes a particularly extreme outlier
  that appears to be 5 times further from the
  center of the distribution than the next furthest observation.
  This is an example of a particularly extreme outlier,
  and for a sample size of only $n = 50$,
  the sample size condition would not be satisfied.
\end{nexample}
\end{examplewrap}
...
deviation,~$\sigma$.
We encountered a similar issue when computing the standard error
for a sample proportion,
and our solution was to use sample value in place
of the population value when computing the standard error.
We'll employ this same strategy for computing the standard
...
and this is a severe enough issue that a fix is required.
% --
%when the sample size is large --
%but it is less reliable when the sample size is smaller
%than about 30. % independent observations.

When working with $\bar{x}$, we'll find it useful to rely
on a new distribution for inference calculations called the
...
When we have a sample size of $n$, we'll use a $t$-distribution
...
\begin{figure}
  \centering
  \Figure{0.65}{tDistConvergeToNormalDist}
...
\footnotetext{
  ...
  in this case a population mean.
