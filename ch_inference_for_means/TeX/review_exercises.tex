...
Project Farm is a YouTube channel that routinely does
comparisons of different products.
In one episode, the channel evaluated different
penetrating oils and options for loosening rusty
...
\begin{center}
\includegraphics[width = 0.8\textwidth]{ch_inference_for_means/figures/eoce/torque_on_rusty_bolt/torque_on_rusty_bolt_dot_plot}
\end{center}
\begin{parts}
\item\label{torque_on_rusty_bolt_appropriate}
    Do you think it is reasonable to apply ANOVA in this case?
\item
    Regardless of your answer from
...
\item\label{torque_on_rusty_bolt_pvalues}
    The table below are p-values or pairwise $t$-tests
    comparing each of the different groups.
    These tests assume the variability across the groups
    is the same, so the test is slightly different than
    $t$-tests we've compared in this book, but the p-value
    ...
    \end{center}
\item
    There are 28 p-values shown in the table from
    part~(\ref{torque_on_rusty_bolt_pvalues}).
    Were any of them statistically significant after
