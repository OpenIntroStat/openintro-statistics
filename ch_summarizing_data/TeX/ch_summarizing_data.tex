...
\Comment{All content in this chapter (previously in Chapter~1)
    was overhauled with new examples and data.
    The structure itself has not changed too much.}
...
\begin{examplewrap}
\begin{nexample}{We might like to compute the average income
...
\begin{figure}[h]
  \centering
  \Figure{0.76}{singleBiMultiModalPlots}
...
We call the distance of an observation from its mean its \term{deviation}. Below are the deviations for the $1^{st}_{}$, $2^{nd}_{}$, $3^{rd}$, and $50^{th}_{}$ observations in the \var{loan\us{}amount} variable.
...
\Comment{We'll be revisiting the jitter added to the
    vertical dot plot in
    Figure~\ref{loan_int_rate_box_plot_layout}.
    We may do something more like a stacked dot plot.}
...
\Comment{The next Guided Practice hits on one possible
  distinction between median / mean:
  use the median if we care about
...
\begin{exercisewrap}
\begin{nexercise}
What interesting features are evident in the \var{median\us{}hh\us{}income} intensity map in Figure~\ref{countyMedIncomeMap}?\footnote{Note: answers will vary. There is a very strong correspondence between high earning and metropolitan areas. You might look for large cities you are familiar with and try to spot them on the map as dark spots.}
\end{nexercise}
\end{exercisewrap}
...
\begin{tabular}{lc}
  ...
  \hline
\end{tabular}
\caption{A frequency table for the \var{homeownership}
    variable.}
...
The \termsub{row proportions}{contingency table!row proportions}
...
\Comment{These next exercises must be checked \textbf{very}
...
\Comment{This next paragraph is heavily revised,
    and the following example is new.}
...
by the levels of \var{app\us{}type}.
...
\Comment{The language against pie charts has been softened.}
...
\begin{figure}[h]
  \centering
  \Figure{1.00}{loan_homeownership_pie_chart}
