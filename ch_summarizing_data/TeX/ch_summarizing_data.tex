...
For example, the value 3496 corresponds to the number of
...
(e.g. $3858/10000=0.3858$ for \resp{rent}),
...
Figure~\ref{rowPropSpamNumber} shows the row proportions for Figure~\ref{loan_home_app_type_totals}. The \termsub{row proportions}{contingency table!row proportions} are computed as the counts divided by their row totals. The value 149 at the intersection of \resp{spam} and \resp{none} is replaced by $149/367=0.406$, i.e. 149 divided by its row total, 367. So what does 0.406 represent? It corresponds to the proportion of spam emails in the sample that do not include any numbers in the message.
...
A \termsub{segmented bar plot}{bar plot!segmented bar plot} is a graphical display of contingency table information. For example, a segmented bar plot representing Figure~\ref{colPropSpamNumber} is shown in Figure~\ref{loan_app_type_home_seg_bar}, where we have first created a bar plot using the \var{number} variable and then divided each group by the levels of \var{spam}. The column proportions of Figure~\ref{colPropSpamNumber} have been translated into a standardized segmented bar plot in Figure~\ref{loan_app_type_home_seg_bar_standardized}, which is a helpful visualization of the fraction of individual loan applications for borrowers in each level of \var{homeownership}.

\begin{figure}[h]
\centering
\subfigure[]{
\includegraphics[width=0.46\textwidth]{ch_summarizing_data/figures/loan_app_type_home_seg_bar/loan_app_type_home_seg_bar}
\label{loan_app_type_home_seg_bar}
}
\subfigure[]{
\includegraphics[width=0.46\textwidth]{ch_summarizing_data/figures/loan_app_type_home_seg_bar/loan_app_type_home_seg_bar_standardized}
\label{loan_app_type_home_seg_bar_standardized}
}
\caption{\subref{loan_app_type_home_seg_bar} Segmented bar plot for numbers found in emails, where the counts have been further broken down by \var{spam}. \subref{loan_app_type_home_seg_bar_standardized} Standardized version of Figure~\subref{loan_app_type_home_seg_bar}.}
\label{loan_app_type_home_seg_bar_plot}
\end{figure}

\begin{example}{Examine both of the segmented bar plots. Which is more useful?}
Figure~\ref{loan_app_type_home_seg_bar} contains more information, but Figure~\ref{loan_app_type_home_seg_bar_standardized} makes it easier to see whether the distributions differ across groups. This second plot makes it clear that joint applications are relatively more common when the applicant has a mortgage than if they rent or own.
\end{example}

Since the proportion of spam changes across the groups in Figure~\ref{loan_app_type_home_seg_bar_standardized}, we can conclude the variables are dependent, which is something we were also able to discern using table proportions. Because both the \resp{none} and \resp{big} groups have relatively few observations compared to the \resp{small} group, the association is more difficult to see in Figure~\ref{loan_app_type_home_seg_bar}.
...
\centering
\subfigure[]{
\includegraphics[width=0.3934\textwidth]{ch_summarizing_data/figures/loan_app_type_home_mosaic_plot/loan_home_mosaic}
\label{loan_home_mosaic}
}
\subfigure[]{
\includegraphics[width=0.46\textwidth]{ch_summarizing_data/figures/loan_app_type_home_mosaic_plot/loan_app_type_home_mosaic}
