...
16924 & 93.30 & 27.00 & F & DC \\ 
   \hline
\end{tabular}
\caption{Six cases from the \data{run10} data set.}
\label{run10DF}
\end{table}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{ch_inference_foundations/figures/run10SampHistograms/run10SampHistograms} 
...
\begin{figure}
   \centering
   \includegraphics[width=0.7\textwidth]{ch_inference_foundations/figures/netTimeRunningMean/netTimeRunningMean}
...
\begin{figure}
   \centering
   \includegraphics[width=0.85\textwidth]{ch_inference_foundations/figures/netTime1000SamplingDistribution/netTime1000SamplingDistribution}
...
\begin{figure}[hht]
   ...
   \caption{Twenty-five samples of size $n=100$ were taken from the \data{run10} data set. For each sample, a confidence interval was created to try to capture the average 10 mile time for the population. Only~1 of these~25 intervals did not capture the true mean, $\mu = 94.52$ minutes.}
...
\begin{example}{If the sample mean of times from \data{run10Samp} is 95.61 minutes and the standard error, as estimated using the sample standard deviation, is 1.58 minutes, what would be an approximate 95\% confidence interval for the average 10 mile time of all runners in the race? Apply the standard error calculated using the sample standard deviation ($SE=\frac{15.78}{\sqrt{100}} = 1.58$), which is how we usually proceed since the population standard deviation is generally unknown.}
