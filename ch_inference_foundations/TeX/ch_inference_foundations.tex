...
Throughout the next few sections we consider a data set called \data{yrbss}, which represents all 13,583 high school students in the Youth Risk Behavior Surveillance System (YRBSS) from 2013.\footnote{\urlwofont{http://www.cdc.gov/healthyyouth/data/yrbs/data.htm}} Part of this data set is shown in Table~\ref{yrbssDF}, and the variables are described in Table~\ref{yrbssVariables}.
...
1 &  14 & female & 9 &  &  & never &   4 &   0 \\ 
  ...
  \hline
\end{tabular}
\caption{Six cases from the \data{yrbss} data set. Some observations are blank since they are missing data. For example, the height and weight of students 1 and 2 are missing.}
\label{yrbssDF}
\end{table}
...
\Comment{Do extensive searching for ``run'' in the LaTeX to find forgotten items to correct.}

\Comment{Figure~\ref{yrbssSampHistograms} is referenced in 4.2. Make sure that text is appropriately updated.}
...
We would like to estimate four features of the high schoolers in YRBSS using the sample. \Comment{Added a 3rd and 4th item.}
\begin{itemize}
...
\Comment{I used ``judgement'' above instead of ``judgment''. The former is the British spelling, and the latter is the American... I think I've always used it with the `e'. If you feel strongly, express an opinion!}
...
\Comment{The next two term boxes were notably changed.}
...
\Comment{Mine, FYI that the next exercise was changed, and it was referenced in EOCE 4.9.}
...
\Comment{This subsection is on the chopping block.}
...
We'll also consider sleep behavior. A recent study found that college students average about 7~hours of sleep per night.\footnote{\urlwofont{http://theloquitur.com/?p=1161}} However, researchers at a rural college are interested in showing that their students sleep longer than seven hours on average. We investigate this topic in Section~\ref{pValue}.
...
\item[$H_A$:] The average days per week that YRBSS students lifted weights was \emph{different} for 2011 than~in~2013.
\end{itemize}
...
%%\begin{example}{Next consider whether the \data{yrbss\_samp} data set provides strong evidence that the average days active per week is different in 2011 and 2013. In 2011, the average was 3.92~days per week, and in the 2013 \data{yrbss\_samp} data set, the average was 2.75 years with a standard deviation of 2.56~days for 100 students.}
%\begin{example}{Next consider whether there is strong evidence that the average age of runners has changed from 2006 to 2012 in the Cherry Blossom Run. In 2006, the average age was 36.13 years, and in the 2012 \data{yrbss\_samp} data set, the average was 35.05 years with a standard deviation of 8.97 years for 100 runners.}
...
\Comment{The college housing data set was updated to have 175 observations and have stronger skew.}
...
\Comment{Additional box on skew since this is such a frustrating topic for students.}
...
\begin{example}{The sample mean for student housing is \$616.91 and the sample standard deviation is \$128.65. Construct a 95\% confidence interval for the population mean and evaluate the hypotheses of Exercise~\ref{htForHousingExpenseForCommunityCollege650}.}
...
\begin{example}{The second college randomly samples 122 students and finds a mean of $\bar{x} = 6.83$ hours and a standard deviation of $s=1.8$ hours. Does this provide strong evidence against $H_0$ in Exercise~\ref{2ndSchSleepHypSetupExercise}? Use a significance level of $\alpha=0.05$.}
...
\Comment{Updated the next caution box and added the paragraph following it.}
...
\end{example}
%library(openintro); library(xtable); data(yrbss); data(yrbss.samp); (x <- by(yrbss.samp$time, yrbss.samp$gender, mean)); diff(x); s <- by(yrbss.samp$time, yrbss.samp$gender, sd); n <- by(yrbss.samp$time, yrbss.samp$gender, length); sqrt(sum(s^2/n))
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{ch_inference_foundations/figures/power132And141/power132And141}
