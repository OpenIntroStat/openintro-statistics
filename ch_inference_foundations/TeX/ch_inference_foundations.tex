...
\begin{figure}[hht]
   ...
   \caption{Twenty-five samples of size $n=100$ were taken from the \data{yrbss} data set. For each sample, a confidence interval was created to try to capture the average 10 mile time for the population. Only~1 of these~25 intervals did not capture the true mean, $\mu = 94.52$ minutes.}
...
\begin{example}{If the sample mean of times from \data{yrbss\_samp} is 95.61 minutes and the standard error, as estimated using the sample standard deviation, is 1.58 minutes, what would be an approximate 95\% confidence interval for the average 10 mile time of all runners in the race? Apply the standard error calculated using the sample standard deviation ($SE=\frac{15.78}{\sqrt{100}} = 1.58$), which is how we usually proceed since the population standard deviation is generally unknown.}
...
\end{example}
% library(openintro); library(xtable); data(yrbss); data(yrbss.samp); mean(yrbss.samp$time); sd(yrbss.samp$time); sd(yrbss$time, na.rm=TRUE)
...
\end{exercise}
%library(openintro); data(yrbss.samp); mean(yrbss.samp$age); sd(yrbss.samp$age)
...
In Section~\ref{confidenceIntervals}, confidence intervals were introduced as a way to find a range of plausible values for the population mean. Based on \data{yrbss\_samp}, a 95\% confidence interval for the 2012 population mean, $\mu_{12}$, was calculated as
\begin{eqnarray*}
(92.45, 98.77)
\end{eqnarray*}
