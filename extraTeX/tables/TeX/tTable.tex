...
\Comment{Discuss picking the smaller $df$ row when the desired
row isn't available.}
...
Just like a normal probability problem, we first draw the picture in Figure~\ref{tDistDF18LeftTail2Point10appendix} and shade the area below -2.10. To find this area, we identify the appropriate row: $df = 18$. Then we identify the column containing the absolute value of -2.10; it~is the third column. Because we are looking for just one tail, we examine the top line of the table, which shows that a one tail area for a value in the third row corresponds to 0.025. About 2.5\% of the distribution falls below -2.10. In the next example we encounter a case where the exact $t$ value is not listed in the table.
\end{nexample}
\end{examplewrap}
...
\label{tDistDF18LeftTail2Point10appendix}
\end{figure}

\begin{examplewrap}
\begin{nexample}{A $t$-distribution with 20 degrees of freedom is shown in the left panel of Figure~\ref{tDistDF20RightTail1Point65appendix}. Estimate the proportion of the distribution falling above 1.65.}
...
\begin{nexample}{Find the value of $t_{18}^{\star}$
  ...
  Identifying $t_{18}^{\star}$ is similar to how we find
  $z^{\star}$ using a table.
  \begin{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{extraTeX/tables/figures/tTails/tTails}
