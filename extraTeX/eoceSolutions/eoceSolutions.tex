...
% 35
\eocesol{\vspace{-4mm}\\\includegraphics[width = 0.4\textwidth]{ch_intro_to_data/figures/eoce/stats_scores_box/stats_scores_boxplot}}
...
\includegraphics[width=0.2\textwidth]{ch_probability/figures/eoce/swing_voters/swing_voters} \\
(c)~24\%.
...
\begin{center}
\includegraphics[width=0.48\textwidth]{ch_probability/figures/eoce/tree_drawing_box_plots/tree_drawing_box_plots}
\end{center}
...
\begin{center}
\includegraphics[width=0.45\textwidth]{ch_probability/figures/eoce/tree_hiv_swaziland/tree_hiv_swaziland}
\end{center}
...
\begin{center}
\includegraphics[width=0.48\textwidth]{ch_probability/figures/eoce/tree_lupus/tree_lupus}
\end{center}
...
% 41
\eocesol{A fair game has an expected value of zero. From the friend's perspective: -$\$5\times 0.46 + x\times 0.54 = 0$. Solving for $x$: \$5.87. You would bet \$5.87 for the Padres to make the game fair.}
...
% 31
\eocesol{Want to find the probabiliy that there will be 1,786 or more enrollees. Using the normal model: 0.0582. With a 0.5 correction: 0.0559.}
...
H_A&: \mu > 2~hours
\end{align*}
The one-sided test indicates that we are only interested in showing that 2 is an underestimate. Here the interest is in only one direction, so a one-sided test seems most appropriate. If we would also be interested if the data showed strong evidence that 2 was an overestimate, then the test should be two-sided.}
...
(d)~See below:
\begin{center}
%\includegraphics[width=0.48\textwidth]{ch_inference_foundations/figures/eoce/penniesWeights/penniesWeights_sketch}
\end{center}
...
(b)~Two different answers are reasonable. $^{Option~1}$Since the population distribution is only slightly skewed to the right, even a small sample size will yield a nearly normal sampling distribution. We also know that the songs are sampled randomly and the sample size is less than 10\% of the population, so the length of one song in the sample is independent of another.  We are looking for the probability that the total length of 15 songs is more than 60 minutes, which means that the average song should last at least $60/15 = 4$ minutes. Using $SE=1.63/\sqrt{15}$, $Z=1.31$ $\to$ 0.0951. $^{Option~2}$Since the population distribution is not normal, a small sample size may not be sufficient to yield a nearly normal sampling distribution. Therefore, we cannot estimate the probability using the tools we have learned so far.
...
\end{align*}
The sample is random, all expected counts are above 5, and $df=(3-1)\times(2-1) = 2 > 1$, so we may proceed with the test.} %The test statistic may be computed as $\chi^2 = 3.1291$ with $df= 2$. The p-value is greater than 0.2, indicating that the evidence is not sufficiently strong for us to reject the null hypothesis. That is, the data do not provide sufficiently strong evidence to conclude that Facebook users knowledge about privacy settings varies by gender.}
...
(e)~63.3302. Since $R^2$ is low, the prediction based on this regression model is not very reliable.
