...
The following three questions come from Hans Rosling's book
\oiRedirect{amazon_factfulness}{Factfulness},
which focuses on adjustin how we see the world:
...
Write down what your answers (or guesses) for each question,
and when you're ready to learn the answers, look in the
footnote.\footnote{1 - c. 2 - c. 3 - a.
If you are a typical person, you got 1 and 2 wrong and 3 correct.}
When looking at a larger sample of people who answer these questions,
it's apparent that some of these questions are harder than others:
the fraction of people who get the correct answer varies dramatically
from one question to the next.

In this section,
we'll be exploring how college-educated people perform
on these questions,
and we'll make conclusions about their knowledge
of these global health topics.

\newcommand{\roslingAsize}{65}
\newcommand{\roslingAprop}{0.246}
\newcommand{\roslingApropcomplement}{0.754}
\newcommand{\roslingApercent}{24.6}
\newcommand{\roslingApercentcomplement}{75.4}
\newcommand{\roslingAcount}{16}
\newcommand{\roslingAcountcomplement}{49}
\newcommand{\roslingAse}{0.053}
...
on the first question on how extreme poverty has changed
...
mean that the typical person knows about public health
trends. It would also be very interesting if we learned
that people do \emph{worse} than random guessing,
since that would suggest people have acquired
...
When considering the first question posed by Rosling,
the null hypothesis represents the notion that people
are as good as random guessing on each of the questions.
That is, the proportion
$p$ of college-educated adults who pick the correct
answer -- that extreme poverty has dropped over the
last 20~years -- is about 33.3\% (or 1-in-3 if wanting to be
perfectly precise).
...
So we are still left with a nagging
concern: is this deviation of \roslingApercent{}\%
...
\begin{example}{Check whether it is reasonable to construct
  ...
  about extreme poverty is between 14.2\%
  and 35.0\%.
\end{example}
...
That is, we fail to reject the null hypothesis, $H_0$,
and the data do not provide sufficient evidence to reject
the notion that the performance of adults was any different
than random guessing.

\begin{example}{Can we conclude that college-educated
    adults simply guess on the extreme poverty question?}
  No, and for two reasons.

  First, when we fail to reject $H_0$, that does not
  necessarily mean we are backing the null hypothesis
  conclusion. Remember the US jury example? If they do not
  find the defendant guilty, that does not mean the jury
  is convinced of their innocence, but rather, they were
  not convinced of the person's guilt.

  Second, we are only evaluating the proportion, not what the
  proportion represent. While 33.3\% represent random guessing,
  perhaps adults had a guess based on their past knowledge,
  but that past knowledge simply wasn't very useful on this
  question and so most of them got it wrong. Some may have even
  known the answer and got it right, while others may have
  thought they knew the answer and got it wrong.
\end{example}
...
The correct answer is 2~billion. While the world population
is projected to increase, the average age is also expected
to rise. That is, the majority of the population growth will
...
\newcommand{\roslingBprop}{0.171}
\newcommand{\roslingBpropcomplement}{0.829}
\newcommand{\roslingBpercent}{17.1}
\newcommand{\roslingBpercentcomplement}{82.9}
\newcommand{\roslingBcount}{39}
\newcommand{\roslingBcountcomplement}{189}
\newcommand{\roslingBse}{0.025}

\begin{exercise}\label{roslingB_normality}
From a (new) random sample of 228 college-educated adults,
...
case?\footnote{Check independence
and the success-failure condition:
\begin{description}
\item[Independence.] Since the data are from a simple
    random sample, the observations are independent.
\item[Success-failure.] We'll use $\hat{p}$ in place of $p$
    ...
    is satisfied.
\end{description}
Since both conditions are satisfied, it is reasonable to use
...
\begin{example}{Compute a 95\% confidence interval for the
    ...
    Practice~\ref{roslingB_hypothesis_setup}.\footnote{}}
...
This last conclusion is not a fluke: Hans Rosling has
an assortment of similar questions to those two we've so
far discussed.
On many of the questions, people actually do worse than
random guessing.
...
However, there are a couple reasons why we would like
a more general tool:
\begin{itemize}
\item Suppose the null value is very far outside of the
    interval, so we reject $H_0$.
    However, we want to communicate that, not only did we
    reject the null hypothesis, but it wasn't even close.
    Such a case is depicted in Figure~\ref{whyWeWantPValue}.
    It would be nice to quantify the strength of this
    relationship.
\item In several sections, we will encounter situations where
...
evidence and work in more complex data scenarios.
...
\begin{example}{Pew Research asked a random sample of 1000 American
    adults whether they supported the increased usage of coal.
    Set up hypotheses to evaluate whether this represents
...
These changes aren't dramatic, but they will require paying
a little more attention to how we use the null value, $p_0$.
...
\begin{example}{Pew Research's sample found that
  ...
  and standard error of \pewcoalpollnullse{}.
...
\caption{
  ...
  $\hat{p} = \pewcoalpollprop{}$.
  To say how unlikely, we use the tail area.}
\label{normal_dist_mean_500_se_016}
\end{figure}

\begin{tipBox}{
  \tipBoxTitle{Checking success-failure and computing
      $SE_{\hat{p}}$ for a hypothesis test}
...
and then we find a tail area in that distribution.
...
\begin{example}{If the null hypothesis were true,
    ...
    as far into the tails \pewcoalpollprop{}
  ...
  the tail area.
  If using the probability table in
  Appendix~\ref{normalProbabilityTable},
  the Z-score $Z = -8.125$ is off the table, so we will
  ...
  we double the lower tail to get the p-value: 0.0004.
...
\begin{example}{A simple random sample of
  ...
  \item[Success-failure.] In a one-proportion hypothesis test,
      the success-failure condition is checked using the null
      proportion, which is $p_0 = \gallupnucleararmspollnullvalue{}$
      in this context:
      $n p_0 = n (1 - p_0)
...
Of course, collecting extra data can be costly,
...
\begin{example}{A car manufacturer is considering switching
    to a new, higher quality piece of equipment that will
    more reliably vehicle door hinges.
    They figure that they will save money in the long run
    through purchasing the equipment if it produces hinges
    that flawed less than 0.5\% of the time.
    However, if the hinges are flawed more than 0.5\% of
  ...
  hinges is 1\%, while the alternative is that it the rate
  is different than 1\%.
...
\Comment{Make this next example into an odd-numbered EOCE.}
