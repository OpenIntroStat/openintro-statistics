...
\Comment{Tighten language around
  accuracy / precision / uncertainty,
  and use the term \emph{sampling error} more frequently.}
...
We'll also use some new notation and terminology:
\begin{itemize}
\item
...
the \term{point estimate} of the parameter $p$,
since it is based on the sample.
It is our single best estimate of $p$.
...
The error consists of two aspects:
...
For example, if we were taking a student poll about a
new college stadium, we'd probably get a biased estimate of the
stadium's level of student support by wording the question as,
\emph{Do you support your school by supporting funding
  for the new stadium?}
We try to minimize bias through thoughtful careful data
collection procedures, which is the topic of many books
and courses.
...
proportion is about \pewsolarpollprop{}}.\footnote{Note:
  \pewsolarpollpercent{} written as a proportion would be
  \pewsolarpollprop{}.
...
where 88.7\% of them are "support" \\
\#\ \ \ \ and 11.3\% are "not". \\
pop\us{}size <- 250000000 \\
possible\_entries <- c(rep("support", \pewsolarpollprop{}*pop\us{}size), rep("not", \pewsolarpollpropcomplement{}*pop\us{}size))\\
\# 2.\ Sample \pewsolarpollsize{} entries without replacement. \\
sampled\_entries <- sample(possible\_entries, size = \pewsolarpollsize{}) \\
\# 3.\ Compute p-hat:
count the number that are "support", then divide
by \\
\#\ \ \ \ the sample size. \\
...
\item[Center.]
    ...
    $\bar{x}_{\hat{p}} = \pewsolarpollprop{}0$,
    which is the same as the population proportion.
...
\begin{onebox}{Sampling distributions are
  ...
  the sample proportion as coming from such a hypothetical
  distribution.
...
\end{onebox}
\footnotetext{Technically this formula is the
  standard deviation of $\hat{p}$, and the standard error
  is the estimated version using $\hat{p}$.
  However, to keep terminology simpler --
  since in every case of inference we do not know the value of $p$
...
are independent.
Even if the sample is bigger than 10\% of the population,
assuming independence
will lead to more conservative results than what we would
get using more advanced methods.
...
These distributions look nothing alike.
...
\Comment{Ensure the two full-page figures are on pages
    facing each other for the final print version.}
...
occur more than 2~standard deviations
...
a value of 1.96~standard deviations as the area under the
normal curve within 1.96~standard deviations is closer to~0.95.
This more precise value is what is used to construct
...
that follows the normal distribution is
\begin{eqnarray*}
\text{point estimate}\ \pm\ 1.96\times SE
\end{eqnarray*}
...
\begin{exercisewrap}
\begin{nexercise}
What does 95\% confident mean in this context?\footnotemark
\end{nexercise}
\end{exercisewrap}
\footnotetext{If we took many such samples and computed
...
Is the normal approximation reasonable for the proportion
of US adults who support expanding wind turbines?\footnotemark
\end{nexercise}
\end{exercisewrap}
\footnotetext{We
...
\Comment{Update OpenIntro's site to handle all new redirect links,
    many of which are currently broken in this edition.}
...
on Roslings' question about how extreme poverty has changed
...
When considering Roslings' question about extreme poverty,
...
\Comment{Build the \data{rosling\_\hspace{0.3mm}responses}
data set and put it in the R~package.}

We will use the \data{rosling\_\hspace{0.3mm}responses}
...
\begin{nexample}{Check whether it is reasonable to construct
  ...
  particular question about extreme poverty is between
  12.2\% and 35.8\%.
\end{nexample}
\end{examplewrap}
...
there are many such questions similar
...
\Comment{Jon suggested we use a simulation to introduce
  the p-value. Seems like it could ease some of the concepts.}
