...
%_______________
\newpage\subsection*{Exercises} % Variability in estimates
...
chips that have severe defects.
...
\item\label{comp_chips_quality_ctrl_prop_se_calc_w_pt_est}%
    ...
    using the point estimate from
    part~(\ref{comp_chips_quality_ctrl_prop_pt_est}).
\item
...
\item\label{us_emergency_expense_prop_se_calc_w_pt_est}%
    ...
    using the point estimate from
    part~(\ref{us_emergency_expense_prop_pt_est}).
\item
...
A nonprofit wants to understand the fraction of households with
