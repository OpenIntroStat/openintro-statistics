...
\item[Nearly normal residuals.]
    ...
    it is usually because of outliers or concerns about
    influential points, which we will discuss in greater
    depth in Section~\ref{typesOfOutliersInLinearRegression}.
    An example of non-normal residuals is shown in the second
    panel of Figure~\ref{whatCanGoWrongWithLinearModel}.
...
\item
    ...
    Figure~\ref{summaryStatsOfSATGPAData} shows the sample means
    for the family income and gift aid as \$101,800 and \$19,940,
    respectively.
...
$$ \widehat{aid} = 24.3 - 0.0431 \times family\us{}income $$
Here we have replaced $y$ with $\widehat{aid}$ and $x$ with
$family\us{}income$ to put the equation in context.
The final equation should always include a ``hat'' on the variable
being predicted, whether it is a generic ``$y$''
or a named variable like ``$aid$''.
\end{nexample}
\end{examplewrap}
...
\begin{nexample}{The intercept and slope estimates for
    the Elmhurst data are $b_0 = 24.3$ and $b_1 = -0.0431$.
  ...
  The estimated intercept $b_0 = 24.3$ (in \$1000s)
...
\begin{nexample}{Use the model
  ...
  so we want to calculate the aid for $family\us{}income = 1000$:
  \begin{align*}
  24.3 - 0.0431\times family\us{}income
    = 24.3 - 0.0431\times 1000
    = -18.8
  \end{align*}
...
remains after using the model: $s_{_{RES}}^2 = 22.4$.
