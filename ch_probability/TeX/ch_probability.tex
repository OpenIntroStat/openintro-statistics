...
\begin{figure}
\centering
\includegraphics[height=1.4in]{ch_probability/figures/cardsDiamondFaceVenn/cardsDiamondFaceVenn}
...
(b)~Using the answers from part (a) and Exercise~\ref{drugUseProbOfParentsEqualNotGivenStudents}(b), compute \vspace{-1.5mm}
\begin{eqnarray*}
...
Write out, in formal notation, the probability a randomly selected person who was not inoculated died from smallpox, and find this probability.\footnote{$P($\var{result} = \resp{died} $|$ \var{inoculated} = \resp{no}$) = \frac{P(\text{\var{result} = \resp{died} and \var{inoculated} = \resp{no}})}{P(\text{\var{inoculated} = \resp{no}})} = \frac{0.1356}{0.9608} = 0.1411$.}
\end{exercise}
