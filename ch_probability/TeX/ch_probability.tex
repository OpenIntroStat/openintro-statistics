...
\section[Defining probability (special topic)]{Defining probability (special topic) \sectionvideohref{youtube-rG-SLQ2uF8U&list=PLkIselvEzpM5EgoOajhw83Ax_FktnlD6n}~\sectionslideshref{gdoc_os3_slides_2-1}}
...
\begin{example}{Consider rolling two dice. If $1/6$ of the time the first die is a \resp{1} and $1/6$ of those times the second die is a \resp{1}, what is the chance of getting two \resp{1}s?}\label{probOf2Ones}
...
home.\footnote{(a)~Yes. Each loan is categorized in only one
...
Create a Venn diagram for this setup.\footnote{%
  ...
  \hfill\includegraphics[height=13mm]{ch_probability/figures/loans_app_type_home_venn/loans_app_type_home_venn} \vspace{-13mm}
  \end{minipage}}
\end{exercise}
...
%_________________
\section[Conditional probability (special topic)]{Conditional probability (special topic) \sectionvideohref{youtube-HxEz4ZHUY5Y&list=PLkIselvEzpM5EgoOajhw83Ax_FktnlD6n}~\sectionslideshref{gdoc_os3_slides_2-2}}
...
\begin{exercise}
Use $P($\var{inoculated} = \resp{yes}$) = 0.0392$ and $P($\var{result} = \resp{lived} $|$ \var{inoculated} = \resp{yes}$) = 0.9754$ to determine the probability that a person was both inoculated and lived.\footnote{The answer is 0.0382, which can be verified using Figure~\ref{smallpoxProbabilityTable}.}
\end{exercise}

\begin{exercise}
If 97.54\% of the people who were inoculated lived, what proportion of inoculated people must have died?\footnote{There were only two possible outcomes: \resp{lived} or \resp{died}. This means that 100\% - 97.54\% = 2.46\% of the people who were inoculated died.}
\end{exercise}
...
%_________________
\section[Sampling from a small population (special topic)]{Sampling from a small population (special topic)~\sectionslideshref{gdoc_os3_slides_2-3}}
\label{smallPop}
...
%_________________
\section[Random variables (special topic)]{Random variables (special topic)~\sectionslideshref{gdoc_os3_slides_2-4}}
...
Just as we have done in many previous cases, we use the variance and standard deviation to describe the uncertainty associated with Leonard's monthly returns. To do so, the variances of each stock's monthly return will be useful, and these are shown in Figure~\ref{sumStatOfGOOGXOM}. The stocks' returns are nearly independent.
...
%_________________
\section[Continuous distributions (special topic)]{Continuous distributions (special topic)~\sectionslideshref{gdoc_os3_slides_2-5}}
\label{contDist}
