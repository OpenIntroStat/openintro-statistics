...
\Comment{Need to add an introductory paragraph.}
...
We use \term{table proportions} to summarize joint probabilities for the \data{family\us{}college} sample. These proportions are computed by dividing each count in Figure~\ref{contTableOfFashionPhotos} by the table's total, 792, to obtain the proportions in Figure~\ref{familyCollegeProbTable}. The joint probability distribution of the \var{parents} and \var{teen} variables is shown in Figure~\ref{familyCollegeDistribution}.
...
\hline
\end{tabular}
\caption{Joint probability distribution for the \data{family\us{}college} data set.}
